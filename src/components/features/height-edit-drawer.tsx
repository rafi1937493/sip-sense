...
interface HeightEditDrawerProps {
  isOpen: boolean
  onClose: () => void
}

export function HeightEditDrawer({ isOpen, onClose }: HeightEditDrawerProps) {
  const { user, updateUser, isLoading } = useHydrationStore()
  
  // Local state for CM mode
  const [heightCm, setHeightCm] = useState(user?.height?.toString() || "")
  
  // Local state for FT/IN mode
  const [heightFt, setHeightFt] = useState("")
  const [heightIn, setHeightIn] = useState("")
  
  const [unit, setUnit] = useState<"cm" | "ft">(user?.heightUnit || "cm")

  useEffect(() => {
    if (isOpen && user) {
      setUnit(user.heightUnit || "cm")
      if (user.height) {
        setHeightCm(user.height.toString())
        // Convert CM to FT/IN for the initial state of FT mode
  ...
  const handleSave = async () => {
    ...
    if (unit === "cm") {
      finalHeight = heightCm ? Number(heightCm) : undefined
    } else {
        ...
        <div className="flex flex-col h-full bg-[#f0f2f5]">
          ...
          <div className="bg-gradient-to-b from-[#1e293b] to-[#0d1f3c] rounded-b-[2.5rem] shadow-xl relative overflow-hidden">
            ...
            <div className="px-6 pt-6 pb-10 flex flex-col items-center gap-2">
              <div className="mb-2 flex size-12 items-center justify-center rounded-2xl bg-white/10 backdrop-blur-sm border border-white/15">
                <Ruler className="size-6 text-blue-300" />
              </div>
          ...
          <div className="flex-1 px-6 py-4">
            <div className="flex flex-col gap-5">
                ...
                <div className="flex bg-[#f1f5f9] rounded-[8px] p-1 self-center w-full max-w-[240px]">
                  <button 
                    ...
                    className={cn(
                      ...
                      unit === 'cm' ? "!bg-[#2563eb] !text-white shadow-md" : "bg-transparent text-slate-500 hover:text-slate-600"
                    )}
                  >CM</button>
                  <button 
                    onClick={() => setUnit("ft")}
                    className={cn(
                      "flex-1 py-2 text-[11px] font-black uppercase rounded-[6px] transition-all",
                      unit === 'ft' ? "!bg-[#2563eb] !text-white shadow-md" : "bg-transparent text-slate-500 hover:text-slate-600"
                    )}
                  >FT/IN</button>
                </div>
              </div>
              ...
              {/* Height Input */}
              <div className="flex flex-col gap-2">
                <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Current Height</Label>
                ...
                {unit === "cm" ? (
                  <div className="relative animate-in fade-in zoom-in-95 duration-300">
                    ...
                    />
                    <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[11px] font-black text-blue-500 uppercase">cm</span>
                  </div>
                ) : (
                  <div className="flex gap-3 animate-in fade-in zoom-in-95 duration-300">
                    <div className="relative flex-1">
                      <Input 
                        ...
                        className="h-14 rounded-2xl border-2 border-white bg-white px-6 text-xl font-black text-[#0d1f3c] shadow-sm focus:border-blue-500 transition-all outline-none"
                      />
                      <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[11px] font-black text-blue-500 uppercase">ft</span>
                    </div>
                    <div className="relative flex-1">
                      <Input 
                        type="number"
                        ...
                        className="h-14 rounded-2xl border-2 border-white bg-white px-6 text-xl font-black text-[#0d1f3c] shadow-sm focus:border-blue-500 transition-all outline-none"
                      />
                      <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[11px] font-black text-blue-500 uppercase">in</span>
                    </div>
                  </div>
                )}
              </div>
